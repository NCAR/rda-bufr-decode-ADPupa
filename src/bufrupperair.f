...
C #      2018-08-09 T. Cram - changed input arguments to accept both input     #
C #        and output file names (including full directory path)               #
C #        Configuration file no longer provides input file names or dates.    #
C ##############################################################################
C
...
C           gdas.adpupa.t00z.20100323.bufr
C           gdas.aircft.t00z.20100323.bufr
...
C
       ...
       IF (NARG .LT. 3) THEN
          ...
          WRITE (*,*) 'Usage: bufrupprair.x input_file '//
...
C
        IF (WMODO.EQ.'y'.OR.WBBDO.EQ.'y')  THEN
          IF (IOBSDO.EQ.'n')  THEN
            ...
            OBSGET(2) = 'PIBAL   '
C
C           DATA WRITTEN TO OBSGET(3) AND OBSGET(4) ARE ONLY
...
C
C         READ THE NEXT BUFR MESSAGE FROM INPUT FILE
C
          CALL READNS(IIUNIT,CSUBSET,RECDATE,ISTATUS)
          IF  (ISTATUS.NE. 0 )  THEN    ! END OF DATA FILE
            CALL CLOSBF (IIUNIT)
            CLOSE (IIUNIT)
            EXIT DORECS   ! PRINT STATISTICS FOR PROCESSING THIS FILE
          ENDIF
C
          RECORDS = RECORDS + 1
c
