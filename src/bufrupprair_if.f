...
c        PARAMETER  ( LENINMX=64 )   ! MAXIMUM LENGTH OF INPUT BASE FILE NAMES
        PARAMETER  ( LENINMX=66 )   ! MAXIMUM LENGTH OF INPUT BASE FILE NAMES
...
C
C       CHANGE: subsetted files will be
C           INFILE_RQSTID.txt
C
C ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
C
        NARG=IARGC()
        IF (NARG .LT. 4) THEN
          WRITE (*,*) 'Enter 4 fields separated by spaces'
          WRITE (*,*) 'rindex input_dir/ filename config_file'
          STOP 'Insufficient input given'
        ELSE
          CALL GETARG(1,argv1)
            RINDEX = argv1
c            write(RINDEX,"(A6)") rtemp
          CALL GETARG(2,argv2)
            infilepath = argv2
          CALL GETARG(3,argv3)
            filename = argv3
...
C
        WRITE (*,*) 'opening configuration file: ', CONFILE
C
        IFILTER = 'n'
C
...
C       /glade/p/rda/data/ds351.0/bufr/2018/gdas.aircar.t00z.20180210.bufr
c
        DO  III = 128, 1, -1
        ...
        DO  III = 128, 1, -1
          ...
          ENDIF
        ENDDO
        write(*,*) 'DIRIN, DIROUT: ', DIRIN, DIROUT
        write(*,*) 'INFILES(1): ', INFILES(1)
C
C ##############################################################################
C #                                                                            #
...
c            PRTFILE = DIROUT(1:EXHALE)//DASTAG//'.'//NAMETAG//'.'
c     +                //DATETAG//'.'//TMTAG//'.tmtxt'
c            PRTFILE = DIROUT(1:EXHALE)//filename//'.tmtxt'
            PRTFILE = './'//filename//'_'//RINDEX//'.tmtxt'
          ELSE
c            PRTFILE = DIROUT(1:EXHALE)//DASTAG//'.'//NAMETAG//'.'
c     +                //DATETAG//'.txt'
c            PRTFILE = DIROUT(1:EXHALE)//filename//'.txt'
            PRTFILE = './'//filename//'_'//RINDEX//'.txt'
          ENDIF
        ELSE
          WRITE (*,*) 'assigning PRTFILE _print'
          PRTFILE = filename//'_print'
          PRTFILE = './'//filename//'_'//RINDEX//'_print'
c          PRTFILE = DIROUT(1:EXHALE)//filename//'_print'
c          PRTFILE = DIROUT(1:EXHALE)//INFILES(1)(01:LENEND)//'_print'
C          PRTFILE = DIROUT(1:EXHALE)//INFILES(KNK)(01:LENEND)//'_print'
        ENDIF
        WRITE (*,*)  'DIROUT: ', DIROUT
        WRITE (*,*)  'INFILES(1): ', INFILES(1)(01:LENEND)
C
C ##############################################################################
